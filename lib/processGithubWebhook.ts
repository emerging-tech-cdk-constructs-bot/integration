...
export const X_GITHUB_INSTALLATION_TARGET_ID = "X-GitHub-Installation-Target-Id"; // The unique identifier of the resource where the webhook was created.
...
export async function processGithubWebhook(event: APIGatewayEvent) {
    ...
    if (!(X_GITHUB_INSTALLATION_TARGET_ID in event.headers) || Number.isInteger(event.headers[X_GITHUB_INSTALLATION_TARGET_ID])) {
        throw new Error(`Missing or Invalid "${X_GITHUB_INSTALLATION_TARGET_ID}" in headers`);
    }
